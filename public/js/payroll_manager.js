...
function addEmployeeToTable(employeeId, employeeText, department, jobTitle,
    ...
    // 📝 [بعد الإضافة] تحقق من القيم في الصف المضاف للتو
    const $row = $(`#${rowId}`);
    const savedName = $row.find('.js-name').val();
    console.log(`📝 [بعد الإضافة ${rowId}] القيم المحفوظة:`, {
        savedName: savedName,
        savedEmployeeId: $row.find('.js-employee-id').val(),
        savedDept: $row.find('.js-dept').val(),
        savedReceiptNo: $row.find('.js-receipt-no').val(),
        savedMissionType: $row.find('.js-mission-type').val(),
        domElement: $row.length > 0 ? 'موجود ✓' : 'غير موجود ✗'
    });
...
function calculateRow($row) {
    ...
    if (isOutsideCountry) {
        ...
        // مدينة عادية مع أكثر من يوم: مبيت ثابت 10,000
        console.log('    - 🏙️ مدينة بـ', nights, 'ليلة → تعيين 10000');
    ...
    if (isOutsideCountry) {
        ...
        // تطبيق 50% على المبيت أيضاً إذا كان محدداً
        const isHalf = $row.find('.js-is-half').is(':checked');
        const accommodationFee = isHalf ? (FIXED_ACCOMMODATION * 0.5) : FIXED_ACCOMMODATION;
